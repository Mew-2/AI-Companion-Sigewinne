...
> 生成时间：2026-09-24 19:56:38｜用例集：`memory_recall_cases.json`（sha256:a0c9dc6d7d89）
> 召回 top_k：3（与 main.py:173 `recall_memories(msg, top_k=3)` 一致）
> 总耗时：45.2s｜token 计量：tiktoken cl100k_base（近似 DeepSeek 分词）
...
- 平均每轮注入 22 token 的记忆上下文；
  召回延迟 P95 24ms，相对一次 LLM 调用（秒级）可忽略，**延迟不是瓶颈**。
